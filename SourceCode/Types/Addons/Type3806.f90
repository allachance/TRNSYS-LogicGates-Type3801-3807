Subroutine Type3806
   ...
   !    DESCRIPTION
   !------------------------------------------------------------------------------------
   ! Subroutine Type3806 implements a logical XNOR gate in a TRNSYS simulation. It
   ! evaluates inputs and computes their XNOR result based on the selected mode.
   !
   ! Parameters:
   ...
   !
   ! Outputs:
   !   Returns 0 if the number of true inputs is odd; otherwise returns 1.
   !
   ! Version History:
   !   2025-05-01 – A. Lachance: Original implementation.
   !
   ! Export this subroutine for use in external DLLs.
   !DEC$ATTRIBUTES DLLEXPORT :: TYPE3806
   !------------------------------------------------------------------------------------
   !    VARIABLES
   !------------------------------------------------------------------------------------
   Use TrnsysConstants
   ...
   Integer :: logic_mode, input_count, input_index, true_count
   ...
   output_flag = MOD(true_count, 2) == 1
