Subroutine Type3804
   ...
   !    DESCRIPTION
   !------------------------------------------------------------------------------------
   ! Subroutine Type3804 implements a logical NOR gate in a TRNSYS simulation. It
   ! evaluates inputs and computes their NOR result based on the selected mode.
   !
   ! Parameters:
   ...
   !
   ! Outputs:
   !   Returns 0 if at least one input is true; otherwise returns 1.
   !
   ! Version History:
   !   2025-05-01 – A. Lachance: Original implementation.
   !
   ! Export this subroutine for use in external DLLs.
   !DEC$ATTRIBUTES DLLEXPORT :: TYPE3804
   !------------------------------------------------------------------------------------
   !    VARIABLES
   !------------------------------------------------------------------------------------
   Use TrnsysConstants
   ...
   Do input_index = 1, input_count
      ...
      If (input_flag .EQV. .TRUE.) Then
         output_flag = .TRUE. ! Do not exit to evaluate all inputs for errors
      End If
   End Do
